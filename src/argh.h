...
namespace argh
{
    ...
    public:
        ...
        // If the conversion was not possible, the stream will enter the fail
        // state, and operator bool will return false.
        template <typename T>
    ...
    public:
        ...
        // returns a std::istream that can be used to convert a positional arg
        // to a typed value.
        string_stream operator()(size_t ind) const;

        // same as above, but with a default value in case the arg is missing
        // (index out of range).
        template <typename T>
        string_stream operator()(size_t ind, T&& def_val) const;

        // parameter accessors, give a name get an std::istream that can be used
        // to convert to a typed value. call .str() on result to get as string
        string_stream operator()(std::string const& name) const;

        // accessor for a parameter with multiple names, give a list of names,
        // get an std::istream that can be used to convert to a typed value.
        // call .str() on result to get as string
        // returns the first value in the list to be found.
        string_stream
        operator()(std::initializer_list<char const* const> init_list) const;

        // same as above, but with a default value in case the param was
        // missing. Non-string def_val types must have an operator<<() (output
        // stream operator) If T only has an input stream operator, pass the
        // string version of the type as in "3" instead of 3.
        template <typename T>
        ...
        template <typename T>
        string_stream
        operator()(std::initializer_list<char const* const> init_list,
                   T&& def_val) const;
                              ...
                              int mode /*= PREFER_FLAG_FOR_UNREG_OPTION*/)
    {
        ...
        for (auto i = 0u; i < args_.size(); ++i)
        {
            ...
            if (!(mode & NO_SPLIT_ON_EQUALSIGN))
            {
                ...
                {
                    params_.insert({ name.substr(0, equalPos),
                                     name.substr(equalPos + 1) });
                    continue;
                }
            }
            ...
            if (1 == (args_[i].size() - name.size()) && // single dash
                argh::parser::SINGLE_DASH_IS_MULTIFLAG & mode
                && // multi-flag mode
                !is_param(name)) // unregistered
            {
                ...
                if (!name.empty()
                    && is_param(
                        std::string(1ul, name.back()))) // last char is param
                {
            ...
            // any potential option will get as its value the next arg, unless
            // that arg is an option too in that case it will be determined a
            // flag.
            ...
            // if 'name' is a pre-registered option, then the next arg cannot be
            // a free parameter to it is skipped otherwise we have 2 modes:
            // PREFER_FLAG_FOR_UNREG_OPTION: a non-registered 'name' is
            // determined a flag.
            //                               The following value (the next arg)
            //                               will be a free parameter.
            //
            // PREFER_PARAM_FOR_UNREG_OPTION: a non-registered 'name' is
            // determined a parameter, the next arg
    ...
    inline std::string
    parser::trim_leading_dashes(std::string const& name) const
    {
